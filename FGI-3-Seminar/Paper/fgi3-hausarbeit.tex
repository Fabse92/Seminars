...
\begin{abstract}
  ...
  Dazu werden die Multiagentensysteme als \textit{interpreted systems} modelliert und intern zu \textit{ordered binary decision diagrams} übersetzt. 
\end{abstract}
...
In dem Tool MCMAS werden Multiagentensysteme als \textit{interpreted systems} modelliert (bzw. mit den leicht modifizierten \textit{deontic interpreted systems}) (vgl. \citep{raimondi2006model} S. 28 ff.).
...
\end{align*}
Die Eigenschaften der einzelnen Agenten werden mit dem Tupel $(L_i,Act_i,P_i,t_i)$
...
Die Bestandteile der Tupel werden nun beschrieben:
\begin{itemize}
...
Die Umwelt unterscheidet sich von anderen Agenten darin, dass ihre Zustände global einsehbar sein können. Die Zustände der Agenten verändern sich alle in diskreten Zeitschritten simultan.\medskip

Zusätzlich zu den CTL-Operatoren kann mit \textit{interpreted systems} mit Operatoren der epistemischen Logik gearbeitet werden (vgl. \citep{raimondi2006model} S. 30 ff.).
...
Eine spezifische Belegung aller Zustände im Multiagentensystem wird als eine Welt bezeichnet. Die Menge W aller erreichbaren Welten enthält alle Welten, die in dem Modell durch legitime Übergänge erreichbar sind. In dem assoziierten Modell $M_{IS}$ zum \textit{interpreted system} $IS$ ist die Formel $K_i\phi$ wahr in der Welt $w$ ($M_{IS},w\models K_I\phi$) wenn gilt: In allen Welten, in denen der Agent $i$ die selben lokalen Zustände hat, wie in $w$, gilt $\phi$. Man sagt: Der Agent i \textit{weiß}, dass $\phi$ gilt.
Intuitiv weiß ein Agent etwas, wenn er es allein aus seinen lokalen Zuständen ableiten kann.

\section{Das Tool MCMAS }
\label{mcmas}
Für den Model Checker für Multiagentensysteme MCMAS wird das MAS als \textit{interpreted system} mit Hilfe der Programmiersprache ISPL (Interpreted Systems Programming Language) definiert. Intern verwendet der Model Checker dabei Algorithmen, die auf OBDDs basieren. Dazu wurde der in Kapitel \ref{specical:obbds} vorgestellte Ansatz für weitere Logiken außer CTL erweitert, sodass in den zu verifizierenden Formeln zusätzlich zu den CTL-Operatoren, epistemische Operatoren (siehe \ref{interpreted_systems}) und noch weitere Operatoren genutzt werden, mit denen man beispielsweise eine Strategie beschreiben kann, die ein Agent verfolgt  \cite{lomuscio2009mcmas}.
Für diese Erweiterungen mussten entsprechende boolesche Funktionen zu den Operatoren eingeführt werden (vgl. \citep{raimondi2006model} S. 69 ff.).

\subsection{Eine Beispielanwendung}
Ein Beispiel für eine Anwendung von MCMAS ist das \textit{dining cryptographers problem} (vgl. \cite{chaum1988dining}). Dabei geht es darum anonymen Nachrichtenaustausch zu betreiben.
...
Dieses Szenario kann intuitiv als MAS modelliert werden.
Die Zustände der Umwelt enthalten das Ergebnis der drei Münzen, und lässt nur jeweils zwei der drei Kryptographen Einsicht auf eine der Münzen.
Ein weiterer Zustand enthält die Information, über die Anzahl der Kryptographen, die ungleiche Münzen gesehen haben möchte.
Die Zustände der Kryptographen enthalten die Informationen über die beiden Münzen, die er gesehen hat, ob er bezahlt hat oder nicht und ob die ihm bekannten Münzen die gleiche Seite zeigen. Den letzten Wert teilen sie der Umgebung mit, sobald er feststeht, bzw. falls einer von ihnen gezahlt hat, sagt er das Gegenteil.
...
So wird von Raimondi selbst ein Verfahren vorgestellt, bei dem einer der beiden Modell Checker Akka oder NuSMV genutzt werden, der Nachteil dabei ist das entweder nur Formeln mit temporalen Operatoren (NuSMV) oder epistemischen Operatoren (Akka) verwendet werden können (vgl. \citep{raimondi2006model} S. 69).\medskip
...
\begin{itemize}
	\item \textbf{MABLE} ist eine Programmiersprache zu Spezifikation von MAS. MABLE verwendet für die Verifikation von Formeln den LTL-Model Checker SPIN  \cite{wooldridge2006model}.
	\item \textbf{MCK} arbeitet ähnlich zu MCMAS, es verwendet auch \textit{interpreted systems} und OBDDs \cite{gammie2004mck}. MCMAS implementiert allerdings eine größere Auswahl an logischen Operatoren (vgl. \citep{raimondi2006model} S. 129).
\end{itemize}
...
Das Tool MCMAS ist in der Lage Modell Checking für Multiagentensystem, die als \textit{interpreted systems} beschrieben wurden, durchzuführen, indem Algorithmen verwendet werden, die auf OBBDs aufbauen. Dabei können für die Formeln, die verifiziert werden sollen, eine Vielzahl verschiedener logischer Operatoren verwendet werden. 
Gegenüber Ansätzen, die das Problem auf Model Checking für Systeme ohne Agenten zurückführen, hat das Tool den Vorteil das eine intuitivere und ausdrucksstärkere Sprache verwendet werden kann, zudem ist das Tool effizienter, da keine Übersetzung in einfachere Probleme stattfindet (vgl. \citep{raimondi2006model} S. 129).

\subsection{Ausblick}

Wichtige Aspekte die in dieser Arbeit nicht behandelt wurden sind:
\begin{itemize}
	...
	\item Wie das Konzept des Modell Checking mit OBDDs auf MAS und die weiteren logischen Operatoren erweitert werden kann.
	\item Die Komplexitätsanalyse der Model Checking-Problems für MAS.	
\end{itemize}

Aktuelle Forschungen des Themas befassen sich beispielsweise damit, dass die generierten Gegenbeispiele zu Spezifikationen oft sehr kompliziert sind, sodass versucht wird Gegenbeispiele zu entwickeln, die möglichst einfach von Menschen verstanden werden können.
Andere Probleme sind die Optimierung der Algorithmen und das on-line Model Checking.
